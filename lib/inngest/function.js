...
export const generateMonthlyReports = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    for (const [i, user] of users.entries()) {
      await step.run(`report-for-${user.id}`, async () => {
      ...
      // Add a small delay between emails (2s) to avoid rate-limiting
      if (i < users.length - 1) {
...
export const checkBudgetAlerts = inngest.createFunction(
  ...
  async ({ step }) => {
          ...
          try {
            await sendEmail({
              ...
              react: EmailTemplate({
                ...
                type: "budget-alert",
                data: {
                  percentageUsed: percentUsed.toFixed(1),
            ...
            console.error(
              `❌ Failed to send budget alert for user ${budget.userId}:`,
              err
            );
          } finally {
            // Always update lastAlertSent, even if email failed
    ...
    return { processedBudgets: budgets.length };
  }
);
